...
# remove outliers?
dat <- dat[-347,] #702RB3
#outlier.iqr <- read.csv("outlier_iqr.csv")
#dat <- dat[!dat$imageID  %in% outlier.iqr$imageID,]
...
#14. best model 1 vs. best model 2
G <- -2 * (logLik(m2)[1] - logLik(m1)[1])
## manually input and replace the df of two models
#pchisq(G14, df = abs(xx - xx), lower.tail = FALSE) 
AIC(m1)
AIC(m2)

### HOOCV for the best model
cv.info.1 <- c()
...
cv.info.1 %>% 
  ...
  geom_bar()+
  scale_x_continuous(breaks = seq(-4,4,1), lim = c(-4.5,4.5))+
  ylab("Frequency")
